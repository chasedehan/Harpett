...
var qIOS = function(){
	...
	if(IOSret[0] > MCCcost[0]){
		for(i=0; i < MCCcost.length; i++){
			for(j=0; j < IOSret.length; j++){
...
var qFloat3 = function(){
	...
	qWords = "A $" + totIssue + " bond was just issued by "+ compName + ", that paid " + ibName + " floatation costs of " + flCostR + 
...
//TVM, solve for value of n
	...
	var correctAnswer = n;
	qWords = "How long will it take for a value of $" + pvLump + " to turn into $" + fv + " at a rate of " + r + "% ? (enter as integer)";
...
//TVM, compute FV given other inputs
	...
	qWords = "Given a value of $" + pvLump + " received in " + n + " years at an interest rate of " + r + "%, what is the Future Value? (enter as integer)";
	var valuePass = [correctAnswer, qWords];
	return(valuePass);
};
...
//TVM, compute PV of lump sum
	...
	qWords = "Compute the Present Value of a payment of $" + fv + " received in " +
...
//TVM, compute PV of lump sum
	...
	qWords = "Compute the present value of " + n + " equal annual payments of $" + pmt +
...
//TVM, compute PV of lump sum
	...
	var correctAnswer = pvAnnuity;
	qWords = "Compute the present value of an ordinary annuity receiving " + n + " annual payments of $" + pmt +
...
//Interest Rates: Effective Annual Rate Calculations
	...
	qWords = bankName() + " is offering an account that pays a simple " + r + "%, compounded " + compound + "; what is the effective annual rate?" +
...
//Bonds: compute value of coupon bond
	...
	qWords = "Compute the current market value of a bond with a $" + fv + " face value, maturing in " + n + " years, an interest rate of " +
...
//Bonds: compute value of coupon bond
	...
	var correctAnswer = pvCouponBond;
	qWords = femaleName() + " is considering buying a " + compName() + " bond for $" + (pvCouponBond * 1.05) + ".  This bond has a maturity value of $" + fv +
...
//Bonds: compute value of coupon bond
	...
	var correctAnswer = pvCouponBond;
	qWords = compName() + " just issued bonds with a par value of $" + fv + ", with a " + n + " year term and a coupon rate of " + (pmt/fv*100).toFixed(2) + 
...
var qCouponExp = function() {
	...
	var pvAnnuity = parseFloat(solveAnnuity(n, r, 'X', pmt));
	var pvCouponBond = (pvAnnuity + pvLump).toFixed(2);
	qWords = compName() + " is considering floating a bond offering and is attempting to determine what kind of interest rate they would be paying."+
...
var qYieldQ1 = function() {
	...
	p0 = Math.floor((Math.random()*100)+ 1);											//Just renaming for my sake
	var p1 = (((y/100) * p0) + p0 - div).toFixed(2);			//finds ending value of a given yield (r)

	qWords = femaleName() + " bought " + num + " shares of " + compName() + " one year ago for $" + p0 + " and just sold them for $" + p1 +
...
var qYieldQ2 = function() {
	...
	var y = parseFloat(((Math.random() - 0.5) * 60)).toFixed(1);
	var div = (Math.random() * 10).toFixed(2);			//Generates a value of a dividend
	p0 = pmt;											//Just renaming for my sake
	var p1 = (( (y/100) * pmt) + pmt - div).toFixed(2);			//finds ending value of a given yield (r)

	qWords = "You just sold all your shares of " + compName() + " for $" + p1 +" each that you paid $" + p0 + " for one year ago. You can't seem to remember" +
...
var qYieldQ3 = function() {
	...
	qWords = maleName() + " just sold bonds for $" + p1 + " and paid $" + p0 + " one year ago.  Over the year he received interest payments of $" + 
...
var qYieldQ4 = function() {
	...
	qWords = femaleName() + " bought shares in " + compName() + " one year ago for $" + p0 + " and just sold them for $" + p1 +
...
var qDDM1 = function() {
	...
	qWords = "You are valuing a stock from " + compName() + " in which you are estimating a stock price of $" + pN + " in " + n + " years.  Given the relative riskiness of this stock, " +
...
var qConstG1 = function() {
	...
	qWords = compName() + " just paid a dividend of $" + div0 + " and is expected to grow at " + g + "%.  You have estimated a required return of " +
...
var qConstG2 = function() {
	...
	qWords = maleName() + " recognizes that " + compName() + " just paid a dividend of $" + div0 + " and estimates that their dividend next year will be $" +
...
var qConstG3 = function() {
	...
	var div1 = (div0*(1+(g/100))).toFixed(2);
	var pN = (div1 / ((r/100)-(g/100))).toFixed(2);
	holder = (pN*0.95).toFixed(2);
	correct = ((pN-holder) * fv).toFixed(2);

	qWords = "This morning, you bought " + fv + " shares of " + compName() + " for $" + holder + " each. Right after you bought these shares, their " +
...
var qUnevenGrowth = function() {
	...
	qWords = compName() + " has been paying an annual dividend of $" + div0 + " for the past 5 years, and plans to continue for the next " + n + " years. After that, " +
...
var qCAPM1 = function() {
	...
	qWords = "You are valuing stock in " + compName() + " and are estimating their Beta at " + beta + ", the risk free rate at " + RF + "%, and equity risk premium at " +
...
var qCAPM2 = function() {
	...
	qWords = "Given a Beta of " + beta + ", RF of " + RF + ", and a market equity risk premium of " + RP + ", what is the current value of a stock with a growth rate of " +
...
var qrHat = function() {
	...
	qWords = "What is the expected return on an investment given the following:" +
...
var qStdDev = function() {
	...
	qWords = "What is the standard deviation of an investment given the following:" +
		"<br> outcome 1 probability: " + s1 + "    return: " + r1 + "%" + 
		"<br> outcome 2 probability: " + s2 + "    return: " + r2 + "%" + 
		"<br> outcome 3 probability: " + s3 + "    return: " + r3 + "%" + 
		"<br>(enter rounded to 2 decimal places)";
...
var qCoefVar = function() {
	...
	qWords = "What is the coefficient of variation of an investment given the following:" +
		"<br> outcome 1 probability: " + s1 + "    return: " + r1 + "%" + 
		"<br> outcome 2 probability: " + s2 + "    return: " + r2 + "%" + 
		"<br> outcome 3 probability: " + s3 + "    return: " + r3 + "%" + 
		"<br>(enter rounded to 2 decimal places)";

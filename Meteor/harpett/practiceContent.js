//practiceContent.js
//Functionality for working through practice problems

if (Meteor.isClient) {
  Template.practiceContent.helpers({
    'qListings': function(){ return QuestionList.find();},
    'popContent': function(){
      var currentUserId = Meteor.userId();
      return Content.find({"createdBy":currentUserId});
    },
    //Hmm, I dont know how to call this function that is below.
    //This is a structural thing, need to figure it out - doesn't do anything now.
  ...
  Template.practiceContent.events({
    'click li': function(){ //click handler for when user wants to look at a particular question
      var questionId = this._id;   
      var currentUserId = Meteor.userId();
      Meteor.call("callFromServer", questionId, currentUserId);  //sending question Id to the server

      //clearing out the problem section 
      $(".welcome").empty();
      $(".answer-input").empty();
      $(".question-results").empty();
      $(".answer-input").append("<br><p>Enter your answer:</p><input type='text'><button class = 'submitQ'>submit</button>");
    },

    'click .submitQ': function(){
        var currentUserId = Meteor.userId();
        var holder4 = Content.findOne({"createdBy":currentUserId});
        var correct=holder4.correct;
        var answer;
        //checking to see if a value is entered
        if ($(".answer-input input").val() !== "") {
          answer = $(".answer-input input").val();
    ...
    'keypress input': function(event){ //Just duplicates the event handler for the two 
        if (event.keyCode ===13) {
          var currentUserId = Meteor.userId();
          var holder4 = Content.findOne({"createdBy":currentUserId});
          var correct=holder4.correct;
...
if (Meteor.isServer) {
  Meteor.startup(function() {
    Meteor.methods({
      'callFromServer': function(questionId, currentUserId){
        ...
        var questionCall = QuestionList.findOne({"_id" : questionId});
        var values = eval(questionCall.qFunction);//I'm apparently not supposed to use eval(), 
              //but, its on the server and the only way I know how to do it right now.

        //Emptying out the Content MongoCollection and repopulating for use by client
        Content.remove({createdBy:currentUserId});
        Content.insert({createdBy: currentUserId, qWords: values[1], correct: values[0]});  
        //This method works to pass the values to the client
        //A useful "bug" is that 2 users cannot be logged into the same account or the values will change 
        //as either user clicks a button.
      }
    });
  });
